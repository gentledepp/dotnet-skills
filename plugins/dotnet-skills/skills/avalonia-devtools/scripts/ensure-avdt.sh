...
if [ -z "${ACCELERATE_LICENSE_KEY:-}" ]; then
   echo "WARN: ACCELERATE_LICENSE_KEY environment variable is not set."
   echo "      You need a valid Avalonia Accelerate license key to use DevTools."
   echo "      Obtain your key from: https://portal.avaloniaui.net/signin?returnUrl=%2F"
   echo "      Then set it: export ACCELERATE_LICENSE_KEY=\"your-license-key\""
fi
